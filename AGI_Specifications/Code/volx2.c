...
void extractObjFile()
{
	...
	if( NULL == fread( buffer, objlength, 1, objfile ) ) {
		printf( "Read error!\n" );
		fclose( objfile );
		fclose( outfile );
		free( buffer );
	...
	printf( "\nConverting %s OBJECT file to OBJECT.NEW... ", ctxt[flag] );

	decryptText( buffer, objlength );

	if( NULL == fwrite( buffer, objlength, 1, outfile ) ) {
		printf( "Error!\nWrite error!\n" );
		fclose( objfile );
		fclose( outfile );
		free( buffer );
...
void extractFile(int fileNameNum, int fileNum)
{
	...
	printf( "\nExtracting %s.%d... ", fileNames[fileNameNum], fileNum );
...
void extractAll(int fileNameNum)
{
	...
	printf( "\nExtracting %s files... ", fileNames[fileNameNum] );

	mkdir( fileNames[fileNameNum] );
...
void extractWordsTok()
{
	...
	printf( "\nConverting WORDS.TOK to WORDS.TXT... " );
