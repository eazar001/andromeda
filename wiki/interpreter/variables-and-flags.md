...
| Variable | Semantics |
|---|---|
| `var(0)` | Current room number (parameter to the most recent `new_room` command). Initialized to 0. |
| `var(1)` | Previous room number. |
| `var(2)` | Code of the border touched by EGO: `0` = nothing, `1` = top edge / horizon, `2` = right edge, `3` = bottom edge, `4` = left edge. |
...
| `var(9)` | 1-indexed position of the first word the input parser failed to match in the player's input; 0 if all words parsed successfully. Set only when vocabulary lookup fails [2-6-Interpreter.html]. The 2-2 phrasing ("if = 0, it is the number of the word ... that was not found") reads as inverted but is semantically consistent with this once read as "if non-zero, gives the unparsed-word position; if zero, all words parsed." See [[interpreter/input-parsing]]. |
...
| `var(16)` | VIEW resource ID associated with EGO. |
...
| `flag(5)` | Set on the very first cycle a room is entered; cleared by the interpreter after that cycle's cleanup. |
