...
2. **Read `andromeda-wiki/log.md` if it exists.** Its tail (`## [YYYY-MM-DD] ingest | <chapter>` entries) is the authoritative record of what was actually applied to the wiki, in case the Progress section drifted out of sync with reality. If they disagree, trust `andromeda-wiki/log.md`.
...
When you finish a phase or a chapter, update the Progress section in this file (Edit tool) and append the corresponding entry to `andromeda-wiki/log.md`. Both updates are part of "done"; do not mark a step complete without both.
...
- [x] `andromeda-wiki/` directory + subdirectories (`entities/`, `concepts/`, `interpreter/`, `sources/`) created
- [x] `andromeda-wiki/index.md` seeded with categorized empty sections
- [x] `andromeda-wiki/log.md` seeded with bootstrap entry
...
```
andromeda/
├── CLAUDE.md                    # +3 lines redirecting AGI-format Qs to andromeda-wiki/index.md first
├── WIKI.md                      # NEW — schema + conventions doc
├── andromeda-wiki/              # NEW — Claude-owned wiki
...
2. **Create `andromeda-wiki/` scaffolding** — empty `entities/`, `concepts/`, `interpreter/`, `sources/` directories (with `.gitkeep` if needed), plus seeded `index.md` (empty categorized sections) and `log.md` (one bootstrap entry).
3. **Edit `CLAUDE.md`** — modify the existing `AGI Specification Reference` section (currently under `### AGI Specification Reference` in the Architecture section) to add: "Consult `andromeda-wiki/index.md` first for AGI format questions — it's a distilled, cited knowledge base maintained by this agent. Fall back to `AGI_Specifications/` only when the wiki is silent or when verifying a contested byte-level claim against source-of-truth." Keep the existing "do not read broadly" guidance for the raw spec.
...
**Resumability.** Every chapter ingest appends to `andromeda-wiki/log.md` with the canonical prefix `## [YYYY-MM-DD] ingest | <chapter>` so a fresh agent can `grep "^## \[" andromeda-wiki/log.md | tail` to see exactly where the previous session stopped. The plan file itself is also updated with a checkbox per chapter as we proceed.
...
- **NEW** `andromeda-wiki/index.md`, `andromeda-wiki/log.md`, and the four subdirectories.
...
1. **Coverage** — every chapter in `AGI_Specifications/Specifications/` has a corresponding entry under `andromeda-wiki/sources/` and at least one entity or concept page that cites it.
2. **Discoverability** — `andromeda-wiki/index.md` lists every wiki page; no orphans. Every page is reachable from the index in ≤2 hops.
...
4. **Round-trip** — open a fresh Claude session and ask a format question (e.g., "what's the byte layout of a VIEW cel header?"). Verify the agent reads `andromeda-wiki/index.md` first, finds the right page, and answers from the wiki — without reading `AGI_Specifications/` HTML.
...
- **If the agidev/ScummVM conflict rate is higher than expected**, add a dedicated `andromeda-wiki/conflicts.md` page that lists every flagged contradiction with resolution status, rather than scattering `> [!conflict]` callouts.
